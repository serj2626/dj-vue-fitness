...
const changeShow = (value: number) => {
  ...
  localStorage.setItem("showAction", `${value}`);
};
</script>
...
<template>
  <div class="container row mt-[150px] overflow-x-hidden">
    ...
    <div class="actions flex justify-center items-center gap-10 mt-10">
      <ProfileButton @click="changeShow(1)" :class="{ action__btn_active: showAction === 1 }" title="Настройки" />
      <ProfileButton @click="changeShow(2)" :class="{ action__btn_active: showAction === 2 }" title="Абонементы" />
      <ProfileButton @click="changeShow(3)" :class="{ action__btn_active: showAction === 3 }" title="Тренировки" />
    </div>
